...
- AI 분석: 환경변수 `AI_API_KEY` 설정 시 Groq API 호출, 미설정 시 규칙 기반만
...
| GET | `/api/crisis/active?loginCode=` | 활성 위험 목록 (loginCode 선택) |
| GET | `/api/risk/latest/{loginCode}` | 최신 AI 위험 평가 |
| POST | `/api/guardian/photo-request/{loginCode}` | 피보호자 촬영 요청 (WebSocket) |
